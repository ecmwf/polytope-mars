...
class Path(Feature):
    def __init__(self, feature_config, client_config):
        ...
        if "axis" in feature_config:
            raise ValueError(
                "Trajectory does not have axis in feature, did you mean axes?"
            )  # noqa: E501
    ...
    def get_shapes(self):
        if len(self.points[0]) == 2:
            if self.inflate == "round":
                ...
                raise ValueError(
                    "Inflate must be either 'round' or 'box' for 2D trajectory feature"
                )  # noqa: E501
            return [
                shapes.Path(
                    ["latitude", "longitude"],
            ...
            if self.inflate == "round":
                ...
                raise ValueError(
                    "Inflate must be either 'round' or 'box' for 3D trajectory feature"
                )  # noqa: E501
            if self.axes[2] == "step":
                return [
                    shapes.Path(
        ...
        if "axes" not in feature_config:
            ...
            for point in feature_config["points"]:
                if len(point) != len(feature_config["axes"]):
                    raise ValueError(
                        "Trajectory points must have the same number of values as axes"
                    )  # noqa: E501
