...
class Grid(Feature):
    def __init__(self, feature_config, client_config):
        assert feature_config.pop("type") == "grid"
        if "points" not in feature_config:
            raise KeyError("Grid must have points in feature")
        ...
        if "axes" in feature_config:
            raise ValueError("Grid does not have axes in feature, did you mean axes?")  # noqa: E501

        assert len(feature_config) == 0, f"Unexpected keys in config: {feature_config.keys()}"

        self.area_bb = get_boundingbox_area(self.points)
        logging.info(f"Area of Grid: {self.area_bb} km\u00b2")
    ...
    def parse(self, request, feature_config):
        if feature_config["type"] != "grid":
            raise ValueError("Feature type must be grid")
        if "axes" in feature_config:
            if len(feature_config["axes"]) < 2 or len(feature_config["axes"]) > 3:
                raise ValueError("Grid axes must contain 2 or 3 values, latitude, longitude, and optionally levelist")
            if "step" in feature_config["axes"]:
                raise ValueError(
                    "Grid axes must be latitude and longitude, step can be requested in main body of request"
                )
            if "latitude" not in feature_config["axes"] or "longitude" not in feature_config["axes"]:
                raise ValueError("Grid axes must contain both latitude and longitude")
            if len(feature_config["axes"]) > 3:
                raise ValueError(
                    "Grid axes must contain at most 3 values, latitude, longitude, and levelist"
                )  # noqa: E501
        ...
        if len(feature_config["points"]) != 2:
            raise ValueError("Grid must have only two points in points")  # noqa: E501
        if "axis" in feature_config:
            raise ValueError("Grid does not have axis in feature, did you mean axes?")  # noqa: E501
        if "axes" not in feature_config:
            for point in feature_config["points"]:
                if len(point) != 2:
                    raise ValueError(
                        "For Grid each point must have only two values unless axes is specified"  # noqa: E501
                    )
        else:
            for point in feature_config["points"]:
                if len(point) != len(feature_config["axes"]):
                    raise ValueError("Grid points must have the same number of values as axes")  # noqa: E501
            if "axes" in feature_config:
                if ("levelist" in feature_config["axes"]) and ("levelist" in request):  # noqa: E501
                    raise ValueError("Grid axes is overspecified in request")  # noqa: E501
